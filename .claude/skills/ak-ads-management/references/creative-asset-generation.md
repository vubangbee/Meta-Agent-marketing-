...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/ak-ai-artist/scripts/search.py "product shot ad" --domain examples
python3 ${CLAUDE_PLUGIN_ROOT}/skills/ak-ai-artist/scripts/search.py "lifestyle marketing" --domain style
```

### Step 2: Generate Images

**Standard (Flash)** â€” fast iterations, search-based prompts:
```bash
npx -y -p @mrgoonie/multix@0.2.0 multix gemini generate \
  --prompt "<prompt>" --aspect-ratio <ratio> --size 2K --output <path>
```

**Creative (Pro)** â€” unique art direction, complex scenes:
```bash
npx -y -p @mrgoonie/multix@0.2.0 multix gemini generate \
...
### Brand Consistency
- Define style prompt once: `"dark academia, muted tones, film grain"` â€” reuse across all creatives
...
### Testing Hierarchy
1. **Hook** â€” same body, 3 different openers; measure 3s hold rate
2. **Format** â€” static vs video vs carousel for same offer
3. **Angle** â€” benefit, social proof, fear, urgency
